...
def feishu(webhook: str, content: str) -> Dict[str, Any]:
    success = []
    error = []
    api_list = webhook.replace('，', ',').split(',') if webhook.strip() else []
    for api in api_list:
        json_data = {
            "msg_type": "text",
            "content": {
        ...
        try:
            ...
            if json_data.get('code') == 0:
                ...
                error.append(api)
                print(f'飞书推送失败, 推送地址：{api}, 失败信息：{json_data.get("msg")}')
        except Exception as e:
            error.append(api)
            print(f'飞书推送失败, 推送地址：{api}, 错误信息:{e}')
    return {"success": success, "error": error}
